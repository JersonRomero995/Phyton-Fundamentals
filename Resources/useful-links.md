Official Documentation

Python Software Foundation. (2024). The Python Tutorial.
https://docs.python.org/3/tutorial/index.html
Python Software Foundation. (2024). Python Language Reference.
https://docs.python.org/3/reference/index.html
Python Software Foundation. (2024). The Python Standard Library.
https://docs.python.org/3/library/index.html

Style Guide

van Rossum, G., Warsaw, B., & Coghlan, N. (2001). PEP 8 – Style Guide for Python Code.
https://peps.python.org/pep-0008/
Style Guide for Python Code  https://peps.python.org/pep-0008/ 

Books (Free Online)

Sweigart, A. (2019). Automate the Boring Stuff with Python (2nd ed.).
https://automatetheboringstuff.com/
Severance, C. (2024). Python for Everybody.
https://www.py4e.com/

Learning Platforms

Real Python. (2024). Python Tutorials.
https://realpython.com/
freeCodeCamp. (2024). Learn Python.
https://www.freecodecamp.org/learn/scientific-computing-with-python/

Video Tutorials

Schafer, C. (2024). Python Tutorial for Beginners [Video series]. YouTube.
https://www.youtube.com/@coreyms
freeCodeCamp.org. (2024). Python for Beginners - Full Course [Video]. YouTube.
https://www.youtube.com/watch?v=rfscVS0vtbw

Community Resources

Stack Overflow. (2024). Python Questions.
https://stackoverflow.com/questions/tagged/python
r/learnpython. (2024). Learn Python Subreddit.
https://www.reddit.com/r/learnpython/


Additional Resources
Official Python Resources

Python.org: https://www.python.org/
Python Package Index (PyPI): https://pypi.org/
Python Enhancement Proposals (PEPs): https://peps.python.org/

Interactive Learning

Python Tutor (Visualize code): https://pythontutor.com/
Replit (Online IDE): https://replit.com/
Google Colab: https://colab.research.google.com/

Practice Platforms

LeetCode: https://leetcode.com/
HackerRank: https://www.hackerrank.com/domains/python
Codewars: https://www.codewars.com/